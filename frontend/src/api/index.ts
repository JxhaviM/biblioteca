...
export const dashboardApi = {
  ...
  getStats: async (): Promise<{ success: boolean; dashboard: DashboardStats }> => {
    const res: any = await apiRequest('/reports/dashboard');
    // El backend responde { success, data: { summary, monthlyActivity, popularBooks, loanStatusDistribution, period } }
    const ok = !!res?.success;
    const data = res?.data || {};
    const summary = data?.summary || {};
    const totalBooks = Number(summary.totalBooks || 0);
    const activeLoans = Number(summary.activeLoans || 0);
    const overdueLoans = Number(summary.overdueLoans || 0);
    const totalUsers = Number(summary.totalUsers || 0);

    // Derivar availableBooks si no viene explícito
    const availableBooks = Math.max(0, totalBooks - activeLoans);

    // Derivar géneros populares a partir de popularBooks
    const popularGenres: string[] = Array.isArray(data?.popularBooks)
      ? data.popularBooks
          .map((b: any) => b?.book?.genre)
          .filter((g: any) => typeof g === 'string')
      : [];

    // Convertir monthlyActivity a una lista simple de actividades recientes
    const recentActivity: Array<{ type: string; description: string; timestamp: string }> = Array.isArray(data?.monthlyActivity)
      ? data.monthlyActivity.map((it: any) => ({
          type: 'loan',
          description: `Préstamos del día ${it?._id}: ${it?.loans ?? 0}`,
          timestamp: it?._id ? `${it._id}T00:00:00.000Z` : new Date().toISOString(),
        }))
      : [];
