...
const CreateUserModal: React.FC<CreateUserModalProps> = ({
  ...
  // Usar el hook de búsqueda reutilizable para personas sin cuenta
  const {
    persons,
  ...
  // Debug: indicar que el modal instanció el hook
  console.log('[CreateUserModal] usePersonSearch instantiated', { withoutAccount: true });
  ...
  // Actualizar búsqueda del hook cuando cambie searchTerm
  useEffect(() => {
    searchPersons(searchTerm);
  }, [searchTerm, searchPersons]);

  // Debug: mostrar resumen legible de personas recibidas en el modal
  useEffect(() => {
    try {
      const sample = persons.slice(0, 10).map(p => `${p._id} / ${p.nombre1 || ''} ${p.apellido1 || ''}`.trim());
      console.log('[CreateUserModal] persons recibidos', { length: persons.length, sample });
    } catch {
      // noop
    }
  }, [persons]);
  ...
  const handleSubmitExisting = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // Mostrar credenciales en el modal especializado
        setCredentials({
          username: response.data.credentials.username,
          password: response.data.credentials.password,
          role: response.data.credentials.role,
          personName: selectedPerson ? `${selectedPerson.nombre1} ${selectedPerson.apellido1}` : undefined
        });
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setCredentials({
          ...
          password: response.data.credentials.password,
          role: response.data.credentials.role,
          personName: `${newUserData.nombre1} ${newUserData.apellido1}`
        });
        setShowCredentials(true);
  ...
  const resetForms = () => {
    ...
    setCustomUsername('');
    setSearchTerm(''); // Usar setSearchTerm del hook
            ...
            <form onSubmit={handleSubmitExisting} className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  ℹ️ Solo se muestran personas que no tienen cuenta de usuario
                </p>
              </div>
                      ...
                      >
                        ...
                        <div>
                          <div className="font-medium text-gray-900">
                            {person.nombreCompleto || `🔍 Persona ${person.doc} (Sin nombre registrado)`}
                          </div>
                          <div className="text-sm text-gray-600">
                            📄 {person.doc} • 👤 {person.tipoPersona}
                            {person.grupo && ` • 👥 ${person.grupo}`}
