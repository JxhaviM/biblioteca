...
// Note: doc ya tiene unique: true en el schema, no necesita índice adicional
...
PersonSchema.methods.getDetailedInfo = async function() {
    // Buscar si esta persona tiene un usuario asociado
    const User = require('./user');
    const userInfo = await User.findOne({ 
        personRef: this._id,
        isActive: true 
    }).select('username role estado isActive isMasterSuperAdmin');

    return {
        ...this.getBasicInfo(),
        name: this.getNombreCompleto(), // Campo name para compatibilidad con frontend
        idNumber: this.doc, // Campo idNumber para compatibilidad
        ...
        hasUser: !!userInfo,
        userId: userInfo ? userInfo._id.toString() : null,
        userInfo: userInfo ? {
            nombreUsuario: userInfo.username,
            rol: userInfo.isMasterSuperAdmin ? 'MasterSuperAdmin' : 
                 userInfo.role === 'superadmin' ? 'SuperAdmin' :
                 userInfo.role === 'admin' ? 'Admin' : 'User',
            estado: userInfo.estado || 'Activo',
